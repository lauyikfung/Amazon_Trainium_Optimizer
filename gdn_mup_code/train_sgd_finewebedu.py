...
# -----------------------------------------------------------------------------
def _is_xla_device_config(value: object) -> bool:
    if not isinstance(value, str):
        return False
    return value.lower() in {"xla", "neuron", "trainium"} or value.lower().startswith("xla:")


if _is_xla_device_config(device) or (device == "cuda" and xm is not None and not torch.cuda.is_available()):
...
def _mark_xla_step() -> None:
    if xm is not None and device_type == "xla":
        xm.mark_step()
...
if device == "cuda" and xm is not None and not torch.cuda.is_available():
    device = "xla"
    print("CUDA is not available and torch_xla is importable; using XLA/Trainium device.")
use_xla = _is_xla_device_config(device)
if use_xla:
    if xm is None:
        raise ImportError(
            "device is configured for Trainium/XLA, but torch_xla is not importable. "
            "Install the AWS Neuron PyTorch stack or run with device='cuda'/'cpu'."
        )
    if backend == "nccl":
        backend = "xla"
if ddp:
...
if device_type in ("cpu", "xla") or dtype == "float32":
    ctx = nullcontext()
else:
...
model.config.nanogpt_model_type = model_type

if use_xla and ptdtype != torch.float32:
    model = model.to(device=device, dtype=ptdtype)
else:
    model.to(device)
...
if ddp:
    comm_device = device if backend in ("nccl", "xla") else "cpu"
...
_use_amp_scaler = (dtype == "float16" and device_type == "cuda")
if _use_amp_scaler:
    try:
...
if _is_gdn_model:
    ...
    _gdn_gate_lr_mult = _gdn_gate_lr_mult * math.sqrt(hidden_size_base / model_hidden_size) if mymup else _gdn_gate_lr_mult
    _gdn_scalar_lr_mult = float(extra_config.get("gdn_scalar_lr_mult", 1.0))
    _gdn_scalar_lr_mult = _gdn_scalar_lr_mult * math.sqrt(model_hidden_size / hidden_size_base) if mymup else _gdn_scalar_lr_mult
    ...
    if master_process:
        print(
            ...
            f"gate_lr_mult={_gdn_gate_lr_mult:.1f}x, scalar_lr_mult={_gdn_scalar_lr_mult:.1f}x "
...
if use_xla and zero_stage != 0:
    if master_process:
        print("[WARN] zero_stage=1 with torch ZeRO is not supported on Trainium; forcing zero_stage=0. "
              "Use neuronx_distributed.optim.NeuronZero1Optimizer for ZeRO-1 on Trainium.")
    zero_stage = 0
...
if init_from == "resume":
    ...
    optimizer_state = torch.load(
        ...
        map_location=("cpu" if use_zero1 or device_type == "xla" else device),
        weights_only=False,
    )
...
if compile:
    if device_type == "xla":
        if master_process:
            print("torch.compile is disabled for XLA/Trainium; continuing without compilation.")
    else:
...
if ddp:
    if use_xla:
        pass   # bare model; xm.optimizer_step will all-reduce grads
    else:
...
def _log_gdn_group_stats(raw_model, opt, iter_num, lr):
    if not (wandb_log and master_process):
        return

    w_sq_dev: dict[str, torch.Tensor] = {}
    g_sq_dev: dict[str, torch.Tensor] = {}
    group_lr_mult: dict[str, float] = {}
    scalar_logs_dev: dict[str, torch.Tensor] = {}

    param_lr_mults = {id(p): float(pg.get('lr_mult', 1.0))
                      for pg in opt.param_groups for p in pg['params']}

    zero = torch.zeros((), device=device, dtype=torch.float32)
    for name, param in raw_model.named_parameters():
        g = _gdn_group_name(name)
        w_sq_dev[g] = w_sq_dev.get(g, zero) + param.data.float().pow(2).sum()
        if param.grad is not None:
            g_sq_dev[g] = g_sq_dev.get(g, zero) + param.grad.float().pow(2).sum()
        ...
        if leaf in ('A_log', 'dt_bias'):
            v = param.data.float()
            scalar_logs_dev[f'gdn/scalar/{leaf}_mean'] = v.mean()
            scalar_logs_dev[f'gdn/scalar/{leaf}_std'] = v.std()
            if param.grad is not None:
                scalar_logs_dev[f'gdn/scalar/{leaf}_grad_norm'] = param.grad.float().norm()

    # ONE mark_step + ONE bulk transfer:
    _mark_xla_step()
    log_dict: dict[str, float] = {}
    for g in sorted(set(w_sq_dev) | set(g_sq_dev)):
        log_dict[f'gdn/weight_norm/{g}'] = float(w_sq_dev[g].sqrt().item())
        if g in g_sq_dev:
            grad_norm = float(g_sq_dev[g].sqrt().item())
    ...
    for k, t in scalar_logs_dev.items():
        log_dict[k] = float(t.item())
...
while True:
    ...
    if grad_clip != 0.0:
        ...
        total_norm_dev = torch.nn.utils.clip_grad_norm_(model.parameters(), grad_clip)
    ...
    if iter_num % log_interval == 0:
        ...
        momentum_div = None
        if use_zero1:
            acc_device = device if device_type == "cuda" or device_type == "xla" else "cpu"
        ...
        total_param_norm_sq_dev = torch.zeros((), device=device, dtype=torch.float64)
        for _, param in model.named_parameters():
            total_param_norm_sq_dev = total_param_norm_sq_dev + \
                param.data.float().pow(2).sum().to(dtype=torch.float64)

        momentum_sq = torch.zeros((), device=device, dtype=torch.float64)
        momentum_sq_sq = torch.zeros((), device=device, dtype=torch.float64)

        state_iter = optimizer.optim.state.values() if use_zero1 else optimizer.state.values()
        for state in state_iter:
            momentum_buf = state.get("momentum_buffer") or state.get("exp_avg")
            second_moment = state.get("exp_avg_sq")  
            if momentum_buf is not None:
                momentum_sq = momentum_sq + \
                    momentum_buf.detach().float().pow(2).sum().to(dtype=torch.float64)
            if second_moment is not None:
                momentum_sq_sq = momentum_sq_sq + \
                    second_moment.detach().float().pow(2).sum().to(dtype=torch.float64)

        if use_zero1 and ddp:
            if backend == "xla":
                import torch_xla.core.xla_model as xm
                momentum_sq = xm.all_reduce(xm.REDUCE_SUM, momentum_sq)
                momentum_sq_sq = xm.all_reduce(xm.REDUCE_SUM, momentum_sq_sq)
            else:
                if backend != "nccl":
                    momentum_sq = momentum_sq.cpu()
                    momentum_sq_sq = momentum_sq_sq.cpu()
        ...
        if master_process:
            total_param_norm = float(total_param_norm_sq_dev.sqrt().item())
            momentum_norm   = float(momentum_sq.sqrt().item())
                ...
                wandb.log(
                    {
                        ...
                        "train/grad_norm": float(total_norm_dev.item()) if grad_clip != 0.0 else 0.0,
                        ...
                        "gpu/memory_allocated_MB": (
                            torch.cuda.memory_allocated() / (1024 * 1024) if device_type == "cuda" else 0.0
                        ),
                        "gpu/max_memory_allocated_MB": (
                            torch.cuda.max_memory_allocated() / (1024 * 1024) if device_type == "cuda" else 0.0
                        ),
                    },
                    step=iter_num,
                )
